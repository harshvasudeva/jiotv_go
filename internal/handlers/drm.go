...
func getDrmMpd(channelID, quality string) (*DrmMpdOutput, error) {
	...
	// Upstream: a stale token yields an unplayable URL, so refresh and refetch
	// before building the output. Only valid for live channels.
...
func buildDrmMpdOutput(liveResult *television.LiveURLOutput, channelID, quality string) (*DrmMpdOutput, error) {
	...
	// Upstream: if the requested quality is absent, fall back to any available one.
	if tv_url == "" {
		if bitrates.High != "" {
...
func DRMKeyHandler(c *fiber.Ctx) error {
	...
	c.Request().Header.Set("versionCode", headers.VersionCode413)
